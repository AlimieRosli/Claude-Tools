...
**Status:** Active
**Last Updated:** 2026-09-09 (added the context-mode shared rule: skill scopes + plugin-maintenance separation)
...
| `topic-dashboard` | `/topic-dashboard` | (none — generates `.ai-tmp/topic-dashboard.html`) | Monitoring | Deterministic generator producing one self-contained HTML page of every topic's progress (classification, docs, tracker phases + steps, tests, deployment, open questions, orchestrator ledger state). Run again to refresh; usable without Claude |
...
| `dev-workflow-orchestrator` | `/dev-workflow-orchestrator` (give a requirement, or ask to resume/continue a topic) | (none — delegates stages; writes the orchestrator ledger `<PREFIX>_ORCH.md`) | End-to-end flow | Claude Code-native end-to-end runner: classifies via `topic-init`, delegates every stage to isolated subagents (writers + mandatory verifiers), holds the human gates itself, and keeps a durable gate ledger so a dropped session can resume |
...
- `dev-workflow-orchestrator` is the end-to-end runner (Claude Code native): it walks this same graph, delegating each stage to an isolated subagent and holding the human gates itself. Its durable gate ledger (`<PREFIX>_ORCH.md`) is what lets a dropped session resume — format and protocol in the orchestrator ledger rule.
...
| Sensitive file scope | The AI never reads sensitive files — any file carrying real credentials/tokens/keys/connection info, in any stack (the per-stack file names are listed once in the rule) — for any reason, including "just checking a value" or running a test. Committed config structure without secrets is normal config, not sensitive. All placeholder/env values come from the adopting repo's placeholder reference doc (e.g. `docs/PLACEHOLDER_REFERENCE.md`); missing value → ask the human to add it, never hunt for it in sensitive files; the reference doc itself is read only with human approval (a `permissions.ask` `Read` rule — snippet in the rule). Applies at codebase exploration, plan env/deploy sections, and test execution | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/sensitive-file-scope.md` (LLM-enforced); the backstop is Claude Code `permissions.deny` `Read` rules in the repo's settings (paste-ready snippet in the rule; residual gap: arbitrary subprocess reads — sandbox unavailable on native Windows) — check before relying on them |
| Temporary artifacts | All AI-created scratch scripts, generated data files, and probe outputs go under the repo-root `.ai-tmp/` folder (gitignored), and are deleted when the task (or phase) that needed them finishes — including after failure, where possible. Scope deletion to files the task created; never wipe the folder blindly. Permanent files go to their real locations; no secrets in temp artifacts. Applies at codebase exploration, implementation, and test runs; `workflow-adopt` installs the `.ai-tmp/` gitignore entry on onboarding | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/temporary-artifacts.md` (LLM-enforced); a deterministic write-gate in `.claude/hooks/` may backstop it — check before relying on it |
...
| Progress Tracker sync | After completing each implementation phase, mark it ✅ in the plan doc's Progress Tracker before starting the next phase — so `topic-status` reads accurate data. Also tick each step's checkbox and bump the tracker's `Steps` count (`<ticked>/<total>`) as steps complete, in the same edit | `${CLAUDE_PLUGIN_ROOT}/skills/topic-implement/rules/topic-implementation-execution.md` ("Progress Sync After Every Phase") (LLM-enforced) — no deterministic gate in the plugin |
| No tests between phases | Complete ALL implementation phases before running any post-implementation tests (NEG post-fix, Positive, REG, and the full unit-test suite). The only tests before implementation are SMK and NEG pre-fix. Do not run tests between phases. The full-suite unit run (the repo's test-suite command) is a **single post-implementation gate that runs after ALL phases** — not a per-phase check — so it does not contradict this rule. | `${CLAUDE_PLUGIN_ROOT}/skills/topic-implement/rules/topic-implementation-execution.md` ("No Tests Between Phases"); adopting-repo `AGENTS.md` (if present) principle 4 |
| Unit-test gate (recommended) | When a plan phase touches function-level logic in service/helper/util layers (the repo's helper/util layer — *adapt to your repo*), run the full unit-test suite once (the repo's test-suite command), after ALL implementation phases and before the NEG post-fix pass. Infrastructure-free (mocked dependencies — no DB/cache/server). Recommended, not gating. Skip for pure endpoint/doc/config-only changes. | `topic-test` rule (LLM-enforced) — no deterministic gate in the plugin |
...
| Orchestrator resume | After a dropped orchestrator session, re-enter via the orchestrator skill — it rebuilds from the topic docs + the gate ledger instead of re-asking gates the human already approved | `${CLAUDE_PLUGIN_ROOT}/skills/dev-workflow-orchestrator/rules/orchestrator-ledger.md` |
...
Shared code paths = the repo's shared middleware/helper/util/config directories (*adapt to your repo's layout*), or any file imported by 2+ modules.
...
**How it's recorded:** The test doc includes a `## Staging Post-Deploy Verification` section with `STG-###` cases. Each case targets the staging URL explicitly (`<STG_GATEWAY_URL>/<gateway-prefix>/api/...` — placeholders use the `<UPPER_SNAKE_CASE>` convention — plus any required auth/role headers, confirmed per repo) and records a `**Result:**` line after the staging run. The plan doc's Deployment Status table marks STG as Deployed, which is the trigger to run these cases.
...
| `temporary-artifacts.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | `topic-implement` (implementation + Final Cleanup Sweep deletion), `topic-test` (disposable run data), `topic-init`/`topic-plan` (exploration one-offs), `workflow-adopt` (installs the `.ai-tmp/` gitignore entry) — scratch scripts/files under the repo-root `.ai-tmp/` folder, deleted at task end |
| `runtime-log-handoff.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | `topic-implement` (failure diagnosis), `topic-test` (test-run/service evidence), `topic-init`/`topic-plan` (human-run probes) — the AI never runs the service; runtime evidence is handed over as a file path the AI reads itself, never a chat paste |
| `code-comment-style.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | `topic-implement` (code-writing phases + Final Cleanup Sweep comment-style check) — comments explain the code, never the workflow: no phase/step/plan/progress metadata in code, one short line, no commented-out code or speculative TODOs |
| `context-mode.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | Orchestrator (mode gate before `topic-init`), `hook-fix`, `hook-init`, `self-update`, `todo-board`, `workflow-adopt`, `workflow-self-correct` — every skill declares one scope (`scope:` frontmatter + leading `SCOPE:` sentence: `project-workflow` / `consumer-hook-ops` / `plugin-maintenance` / `workflow-governance`); the plugin source repo defaults to plugin maintenance and never runs the topic flow; a request crossing modes is stopped and re-routed, never blended |
| `stack-content-layer.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | All topic skills, templates, and hooks — the plugin's always-loaded layer is category-shaped ("the cache", "the database", "the API gateway"); per-technology command syntax lives in `skills/_shared/stack/stack-<tech>.md` reference files, loaded on demand per the repo's stack list (a repo-owned file, referenced by the managed boilerplate). The rule itself is the single source of the mechanism and naming convention |
...
| `dev-workflow-orchestrator` | `${CLAUDE_PLUGIN_ROOT}/skills/dev-workflow-orchestrator/rules/orchestrator-ledger.md` | Durable gate ledger (`<PREFIX>_ORCH.md`): verifier results, human approvals, verbatim DECISIONS written at every gate; resume protocol for a dropped session (rebuild from docs + ledger, edge cases) |
...
**`UNIT` column note:** ⚠️ means **recommended** (not required/gating) — generate `UNIT-###` cases when a plan phase touches function-level logic in service/helper/util layers (the repo's helper/util layer — *adapt to your repo*); skip for pure endpoint/doc/config-only changes. The full unit-test suite (the repo's test-suite command) runs **after ALL implementation phases** and **before the NEG post-fix pass** (see the execution flow and `${CLAUDE_PLUGIN_ROOT}/skills/topic-test/rules/topic-test-doc-writing.md`). This is LLM-enforced — no deterministic gate in the plugin.
...
| 2 | Coding standards conformance — the repo's request-layering convention, error-handling pattern per file, use of the repo's structured logger, every `catch` logs + `return` before responding; Reuse field per Phase 1+ is grep-verified (anti-spaghetti) | Adopting-repo `AGENTS.md` coding conventions (if present); `${CLAUDE_PLUGIN_ROOT}/skills/topic-plan/rules/topic-plan-doc-writing.md` (Reuse Existing Code) |
