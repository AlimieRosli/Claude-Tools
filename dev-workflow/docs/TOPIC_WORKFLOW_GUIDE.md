...
| `context-mode.md` | `${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/` | Orchestrator (mode gate before `topic-init`), `hook-fix`, `hook-init`, `self-update`, `workflow-adopt`, `workflow-self-correct` — every skill declares one scope (`scope:` frontmatter + leading `SCOPE:` sentence: `project-workflow` / `consumer-hook-ops` / `plugin-maintenance` / `workflow-governance`); the plugin source repo defaults to plugin maintenance and never runs the topic flow; a request crossing modes is stopped and re-routed, never blended |
