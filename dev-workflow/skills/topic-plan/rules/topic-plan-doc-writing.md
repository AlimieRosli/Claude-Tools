...
- The full unit-test suite (e.g. `npm test`, if the repo has one) runs once, **after ALL implementation phases** and **before the NEG post-fix pass** — the Unit Tests field names the cases, it does not change run order. Phase 0 is exempt.
