...
- **Before specifying a new helper, service function, or utility in a phase step, grep the repo's shared-helper directories (the repo's helpers/utils/services layer) for an existing equivalent.** If one exists, the step must say "reuse `<existing>(...)`" rather than "add a new function." If none exists, state "No existing utility found (verified by grep on <keywords>)."
...
- Every Phase 1+ block in the plan doc that touches **non-trivial function-level logic** in the repo's service/helper/util layers should include a **Unit Tests** field (after Reuse) naming the colocated test file (mirroring the source tree, per the repo's test conventions) and the `UNIT-###` cases that verify it. State `N/A — no service/helper/util logic touched` when the phase is pure wiring, doc, or config-only.
...
- The full unit-test suite (the repo's test-suite command) runs once, **after ALL implementation phases** and **before the NEG post-fix pass** — the Unit Tests field names the cases, it does not change run order. The canonical run-order rule is in [`${CLAUDE_PLUGIN_ROOT}/skills/topic-test/rules/topic-test-doc-writing.md`](${CLAUDE_PLUGIN_ROOT}/skills/topic-test/rules/topic-test-doc-writing.md); Phase 0 is exempt.
...
- **Trigger:** the phase is **mandatory when any code change in the implementation phases touches information documented in the repo's API reference or architecture docs** (e.g. `docs/API_REFERENCE.md` / `docs/ARCHITECTURE.md` if present in the adopting repo — adapt doc names as needed) — for example, a new/changed/removed endpoint, route prefix, request/response field, auth guard, or API version tag (→ API reference); or a change to the boot sequence, the repo's request-layering convention, database connections/models (the repo's connection/model wiring), config keys, middleware order, logging setup, or deployment shape (→ architecture doc).
