...
description: "SCOPE: project-workflow — acts on an adopting repo's code and docs/ref/ topic docs, never on the dev-workflow plugin itself. Topic Test Doc Owner. Writes and maintains the test doc for a topic following the docs/ref/<MODULE>/<TOPIC>/ convention — environment, required test cases (smoke & sanity, negative flow run before AND after the fix, positive flow run after), the recommended unit-tests category (function-level, mocked, run as the repo's full test-suite after all implementation phases and before the NEG post-fix pass), plus optional categories generated only on request (edge, error, regression, performance), expected results, side-effect checks, execution flow, and pass criteria. USE FOR: writing a new test doc after the main and plan docs exist; updating an existing test doc; recording test run results. INVOKE WITH: /topic-test <module-name> <topic-name>"
argument-hint: "<ModuleName> <TopicName>"
---

# Topic Test — Test Doc Owner

> *Adapt paths/commands to your repository's actual layout and tooling (test runner, dev-server command, logger, gateway prefixes). Cache/database command syntax comes from the repo's stack file — see the Stack Content Layer rule.*
...
5. BEFORE Step 6 (any test run — and re-consulted whenever an env value is needed): [`${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/sensitive-file-scope.md`](../../_shared/rules/sensitive-file-scope.md) — never read sensitive files (stack-dependent: env/config files, profile files, keys, credentials); all placeholder/env values come from the adopting repo's placeholder reference doc (e.g. `docs/PLACEHOLDER_REFERENCE.md`).
6. BEFORE running any test case (Step 6): [`${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/temporary-artifacts.md`](../../_shared/rules/temporary-artifacts.md) — synthetic payloads, generated fixtures, and captured probe responses go under the repo-root `.ai-tmp/` folder and are deleted when the run's cases finish.
7. BEFORE Step 8: [`${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/topic-doc-writing-conventions.md`](../../_shared/rules/topic-doc-writing-conventions.md) — the per-prompt and per-session discipline reminders that must be delivered verbatim at the end of the run.
8. BEFORE asking the human for any test-run or service output (Step 6): [`${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/runtime-log-handoff.md`](../../_shared/rules/runtime-log-handoff.md) — the human runs the service and hands over evidence as a file path the AI reads itself — never a chat paste; the AI asks with one concrete command per round-trip.
...
Look at the layers the target repo actually uses — the route/handler layer, business-logic modules, external-call services, shared helpers, config/env setup — plus existing tests (patterns). Map this to the repo's equivalent entry points, service layer, and config surface.

Minimum: 1 grep on topic keywords, 2 source files read, 1 existing test file (if any).

**Local run & logging convention (example — adapt to the repo):** a typical service runs locally via its dev-server command, which loads its **gitignored** env/config file for config vars (stack-dependent — the Sensitive File Scope rule lists the per-stack file names and forbids reading them) and uses a structured logger at `debug` level for non-production runs. When a test needs to inspect runtime state, add a temporary `log.debug` via the repo's logger and read it from the dev-server terminal — **do not use `console.log`** (it bypasses structured/redacted logging and is hard to read in the terminal). Read [`${CLAUDE_PLUGIN_ROOT}/skills/topic-test/rules/topic-test-doc-writing.md`](rules/topic-test-doc-writing.md) now if you have not already (Mandatory reads #2) — its "Environment Scope" rule has the full convention.

**Disposable test data (Mandatory reads #6):** synthetic payloads, generated fixtures, and captured probe responses used by a test run go under the repo-root `.ai-tmp/` folder (per [`${CLAUDE_PLUGIN_ROOT}/skills/_shared/rules/temporary-artifacts.md`](../../_shared/rules/temporary-artifacts.md)) — not next to real fixtures, not in source directories — and are deleted when the run's cases finish.
...
| Unit Tests | `UNIT-` | **Recommended** — when a plan phase touches function-level logic in the repo's service/helper/util layer; skip for pure route/doc/config-only changes | Function-level logic exercised in-process with mocked DB/cache/external calls — fast, infrastructure-free; the repo's full test-suite command runs **after all implementation phases** and **before the NEG post-fix pass** (see `rules/topic-test-doc-writing.md`) |
...
7. **Test Cases — Unit Tests (recommended)** — one `###` per case, generated when a plan phase touches function-level logic in the repo's service/helper/util layer; skip for pure route/doc/config-only changes. Placed immediately after Positive Flow and before Edge Cases (see the template). The repo's full test-suite command runs **after all implementation phases** and **before the NEG post-fix pass**.
...
| **Test Results Dashboard** | After each test case (same edit) | Update the case's row in the `## Test Results Dashboard` table at the top of the doc **in the same edit** as its result line — Status (`☐ Not Run` / `🔄 In Progress` / `✅ PASS` / `❌ FAIL`), NEG pre/post-fix result columns for NEG rows, and `Last Run` date. The case body's result line is the source of truth; a row that disagrees is corrected immediately. Never batch dashboard updates. See "Dashboard is synced with result lines" in the rules file. |
