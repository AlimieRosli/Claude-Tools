...
Used by `topic-init`, `topic-plan`, and `topic-test` to resolve the module and topic names and derive the doc paths. This is a shared reference file, not a skill itself — it has no `SKILL.md` and is only loaded when linked from a skill's own SKILL.md (each skill's Step 1 links here as the single source of truth).
...
2. **Skill-specific editor fallback** — if `$ARGUMENTS` is absent, check the active editor file:
   ...
   - `topic-plan`: active plan doc (`docs/ref/<MODULE>/<TOPIC>/<PREFIX>_PLAN.md`) — derive module and topic from the folder names.
