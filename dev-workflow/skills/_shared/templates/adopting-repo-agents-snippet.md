...
- The plugin references gates **defensively** (e.g. "the plugin's `secret-scan` gate applies when the repo has adopted the hooks — check before relying on them"). Where this repo has not adopted the hooks (no `.claude/hooks.config.json`), the corresponding rule stays LLM-enforced (best-effort).
